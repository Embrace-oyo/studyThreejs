...
`;function b(s){const e={...s};return"defines"in s&&(e.defines={...s.defines}),"uniforms"in s&&(e.uniforms=te.clone(s.uniforms)),e}const ot={defines:{X_IS_EVEN:1,Y_IS_EVEN:1},uniforms:{map:{value:null},originalMapSize:{value:new _},parentMapSize:{value:new _},parentLevel:{value:0}},vertexShader:`
		varying vec2 vUv;
		void main() {
	...
	`},Y=new r;class rt{constructor(e){e||(e=`
			...
			`);const t=b(ot);t.fragmentShader=t.fragmentShader.replace(/<mipmap_logic>/g,e);const i=new Array(4);i[0]=new g(b(t)),i[0].defines.X_IS_EVEN=0,i[0].defines.Y_IS_EVEN=0,i[1]=new g(b(t)),i[1].defines.X_IS_EVEN=1,i[1].defines.Y_IS_EVEN=0,i[2]=new g(b(t)),i[2].defines.X_IS_EVEN=0,i[2].defines.Y_IS_EVEN=1,i[3]=new g(b(t)),i[3].defines.X_IS_EVEN=1,i[3].defines.Y_IS_EVEN=1;const n=new T;n.texture.minFilter=D,n.texture.magFilter=D,this._swapTarget=n,this._copyQuad=new B(new g(ie)),this._mipQuad=new B(null),this._mipMaterials=i}update(e,t,i,n=!1){e.isWebGLRenderTarget&&(e=e.texture);const l=i.autoClear,p=i.getClearAlpha(),A=i.getRenderTarget();i.getClearColor(Y);const c=this._copyQuad,a=this._mipQuad,o=this._swapTarget,M=this._mipMaterials;let m,u;n?(m=V.floorPowerOfTwo(e.image.width),u=V.floorPowerOfTwo(e.image.height)):(m=Math.floor(e.image.width),u=Math.floor(e.image.height));const P=Math.floor(m*1.5),y=Math.floor(u);t.setSize(P,y),o.texture.type!==t.texture.type?(o.dispose(),o.copy(t),o.texture.image={...o.texture.image}):o.setSize(P,y),i.autoClear=!1,i.setClearColor(0),i.setClearAlpha(),c.material.uniforms.tDiffuse.value=e,c.camera.setViewOffset(m,u,0,0,P,y),i.setRenderTarget(t),i.clear(),c.render(i),i.setRenderTarget(o),i.clear(),c.render(i);let I=m,h=u,N=0;for(;I>1&&h>1;){const K=(I%2===0?1:0)|(h%2===0?2:0),x=M[K];x.uniforms.map.value=o.texture,x.uniforms.parentLevel.value=N,x.uniforms.parentMapSize.value.set(I,h),x.uniforms.originalMapSize.value.set(m,u),a.material=x,I=Math.floor(I/2),h=Math.floor(h/2);const q=y-2*h;i.setRenderTarget(t),a.camera.setViewOffset(I,h,-m,-q,P,y),a.render(i),i.setRenderTarget(o),x.uniforms.map.value=t.texture,a.render(i),N++}return i.setRenderTarget(A),i.setClearAlpha(p),i.setClearColor(Y),i.autoClear=l,N+1}dispose(){this._swapTarget.dispose(),this._mipQuad.dispose(),this._copyQuad.dispose(),this._mipMaterials.forEach(e=>e.dispose())}}var lt=`uniform float iTime;
