import{V as u,a1 as A,x as R,W as m,e as p,P as f,w as g,O as U,H as C,I as S,J as w,a2 as z,q as E,y as s,f as I,M as i,s as D,G as y,k as O,l as b,a3 as H,a4 as K}from"./threejs-R2oN4ICD.js";import{M as l,_ as j}from"../lib/index-CADhc86s.js";import{P as B,l as k,b as F,m as L,i as P,c as T,d as c,F as G,n as N,p as $,e as W,q as V}from"./vendor-D10xMT_S.js";const q="/static/obj/SkullHead-ChT8SaEw.obj",J="/static/png/noise-QVA7tH9y.png";var X=`attribute vec3 position;
attribute vec2 uv;
...
}`;function M(v){return new URL(Object.assign({"../assets/SkullHead.obj":q,"../assets/noise.png":J})[`../assets/${v}`],import.meta.url).href}class se{constructor(e){this.parent=e.parent,this.target=e.target,this.callback=e.callback,this.devicePixelRatio=window.devicePixelRatio,this.width=this.target.offsetWidth*this.devicePixelRatio,this.height=this.target.offsetHeight*this.devicePixelRatio,this.resolution=new u(this.width,this.height),this.renderer=new A({powerPreference:"high-performance",alpha:!0,antialias:!0,preserveDrawingBuffer:!0}),this.renderer.setPixelRatio(window.devicePixelRatio),this.renderer.setSize(this.width,this.height),this.renderer.setClearColor(0,1),this.target.appendChild(this.renderer.domElement),this.clock=new R,this.time=0,this.radian=0,this.renderTarget1=new m(256,256),this.renderTarget2=new m(256,256),this.cameraInit(),this.assetsInit(),this.paneInit()}paneInit(){this.pane=new B({container:this.parent}),this.controller=this.pane.addFolder({title:"控制台",expanded:!0}),this.controller.addBinding(this,"distance",{min:.1,max:50,step:.01,label:"光环大小"})}cameraInit(){this.scene=new p,this.sceneAura=new p,this.aspect=this.width/this.height,this.camera=new f(50,this.aspect,.1,1e3),this.camera.setFocalLength(50),this.camera.position.set(0,10,40),this.camera.lookAt(new g(0,0,0)),this.cameraAura=new f(50,1,.1,1e3),this.cameraAura.setFocalLength(50),this.distance=Math.abs(Math.tan(l.radians(50)/2)*2)*20*1.55,this.controls=new U(this.camera,this.renderer.domElement),this.controls.enableDamping=!0}async assetsInit(){this.manager=new C,this.noiseTexture=new S(this.manager).load(M("noise.png")),this.noiseTexture.wrapS=w,this.noiseTexture.wrapT=w,this.model=await new z(this.manager).loadAsync(M("SkullHead.obj")),this.skullHead=this.model.children[1].geometry,this.skullJaw=this.model.children[0].geometry,this.manager.onLoad=()=>{console.info("%c资源加载完成!✅","color:#fff;background-color:red"),this.callback(),this.backgroundInit(),this.auraPostEffectInit(),this.skullInit(),this.auraInit(),this.pointsInit(),this.readyInit(),this.animation(),this.resize()}}cameraUpdate(e){this.time+=e,this.cameraAura.position.copy(this.camera.position).normalize().multiplyScalar(this.distance),this.cameraAura.lookAt(new g)}backgroundInit(){const e=new E(100,12,12),t=new s({uniforms:{time:{value:0},hex:{value:0}},vertexShader:X,fragmentShader:Q,side:I});this.background=new i(e,t),this.background.name="Background"}auraPostEffectInit(){const e=new D(2,2),t=new s({uniforms:{resolution:{value:new u(this.width,this.height)},direction:{value:new u(0,0)},radius:{value:1},texture:{value:this.noiseTexture}},vertexShader:ee,fragmentShader:te});this.auraPostEffect=new i(e,t),this.auraPostEffect.name="AuraPostEffect"}skullInit(){this.skull=new y,this.skullMaterial=new s({uniforms:{time:{value:0},renderOutline:{value:0},noiseTex:{value:this.noiseTexture}},vertexShader:Y,fragmentShader:Z}),this.head=new i(this.skullHead,this.skullMaterial),this.jaw=new i(this.skullJaw,this.skullMaterial),this.skull.add(this.head),this.skull.add(this.jaw),this.skull.name="Skull"}skullUpdate(e){this.skullMaterial.uniforms.time.value+=e,this.head.rotation.set(l.radians(-(Math.sin(this.skullMaterial.uniforms.time.value)*.7+.7)*8),0,0),this.jaw.rotation.set(l.radians((Math.sin(this.skullMaterial.uniforms.time.value)*.7+.7)*8),0,0)}auraInit(){const e=new D(20,20),t=new s({uniforms:{time:{value:0},postEffectTex:{value:this.renderTarget1.texture},noiseTex:{value:this.noiseTexture}},vertexShader:ne,fragmentShader:re,transparent:!0});this.aura=new i(e,t),this.aura.name="Aura"}auraUpdate(e){this.aura.rotation.copy(this.camera.rotation),this.aura.material.uniforms.time.value+=e}pointsInit(){const n=new O,h=new b(new Float32Array(360*3),3),o=new b(new Float32Array(360),1);for(var r=0,a=360;r<a;r++){const x=l.radians(Math.random()*360),d=Math.random()*4+1;h.setXYZ(r,Math.cos(x)*d,0,Math.sin(x)*d),o.setX(r,Math.random()*4)}n.setAttribute("position",h),n.setAttribute("delay",o);const _=new s({uniforms:{time:{value:0},duration:{value:4},resolution:{value:new u(this.width,this.height)},pixelRatio:{value:window.devicePixelRatio},noiseTex:{value:this.noiseTexture}},vertexShader:ie,fragmentShader:oe,transparent:!0,blending:H,depthWrite:!1});this.points=new K(n,_),this.points.name="Points"}pointUpdate(e){this.points.material.uniforms.time.value+=e,this.points.rotation.set(0,this.points.material.uniforms.time.value*.2,0)}readyInit(){this.group=new y,this.group.add(this.skull),this.group.add(this.aura),this.group.add(this.points),this.scene.add(this.group),this.scene.add(this.background)}animation(){this.renderer.setAnimationLoop(()=>this.animation());const e=this.clock.getDelta();this.radian+=e,this.controls.update(),this.cameraUpdate(e),this.skullUpdate(e),this.auraUpdate(e),this.pointUpdate(e),this.renderer.setRenderTarget(this.renderTarget1),this.sceneAura.add(this.skull),this.skullMaterial.uniforms.renderOutline.value=1,this.renderer.render(this.sceneAura,this.cameraAura),this.renderer.setRenderTarget(this.renderTarget2),this.sceneAura.remove(this.skull),this.sceneAura.add(this.auraPostEffect),this.auraPostEffect.material.uniforms.direction.value.set(1,0),this.auraPostEffect.material.uniforms.texture.value=this.renderTarget1.texture,this.renderer.render(this.sceneAura,this.cameraAura),this.renderer.setRenderTarget(this.renderTarget1),this.auraPostEffect.material.uniforms.direction.value.set(0,1),this.auraPostEffect.material.uniforms.texture.value=this.renderTarget2.texture,this.renderer.render(this.sceneAura,this.cameraAura),this.renderer.setRenderTarget(null),this.sceneAura.remove(this.auraPostEffect),this.group.add(this.skull),this.skullMaterial.uniforms.renderOutline.value=0,this.renderer.render(this.scene,this.camera)}resize(){window.addEventListener("resize",()=>{this.width=this.target.offsetWidth,this.height=this.target.offsetHeight,this.resolution.set(this.width,this.height),this.aspect=this.width/this.height,this.renderer.setSize(this.width,this.height),this.camera.updateProjectionMatrix(),this.points.material.uniforms.resolution.value.copy(this.resolution)})}destroy(){this.scene.traverse(e=>{var t;e instanceof i&&((t=e.geometry)==null||t.dispose(),Object.values(e.material).forEach(n=>{n&&typeof n.dispose=="function"&&n.dispose()}))}),this.renderer.dispose(),this.renderTarget1.dispose(),this.renderTarget2.dispose()}}const ae={class:"skull"},ue={class:"load"},le={__name:"index",setup(v){const e=k(!1);k(0);let t=null;const n=()=>{e.value=!0};return F(()=>{t=new se({parent:document.querySelector(".skull"),target:document.querySelector(".canvas"),callback:n})}),L(()=>{t.destroy(),t=null,console.info("%c头骨-销毁😁","color:#fff;background-color:red")}),(h,o)=>(P(),T("div",ae,[c("div",{class:V(["loading",{loadOk:e.value}])},[c("div",ue,[(P(),T(G,null,N("LOADING...",(r,a)=>c("span",{key:a,style:$("--i:"+a)},W(r),5)),64))])],2),o[0]||(o[0]=c("div",{class:"canvas"},null,-1))]))}},xe=j(le,[["__scopeId","data-v-fcfd62b0"]]);export{xe as default};
