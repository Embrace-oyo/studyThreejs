...
float snoise(vec3 v) {
    ...
    return 42.0 * dot(m * m, vec4(dot(p0, x0), dot(p1, x1),
                dot(p2, x2), dot(p3, x3)));
}`;function C(u){return new URL(Object.assign({"../assets/cubeMap1/config.json":me,"../assets/cubeMap1/negx.png":ge,"../assets/cubeMap1/negy.png":ve,"../assets/cubeMap1/negz.png":xe,"../assets/cubeMap1/posx.png":fe,"../assets/cubeMap1/posy.png":ye,"../assets/cubeMap1/posz.png":Ae,"../assets/cubeMap2/negx.png":Ce,"../assets/cubeMap2/negy.png":we,"../assets/cubeMap2/negz.png":Ie,"../assets/cubeMap2/posx.png":Pe,"../assets/cubeMap2/posy.png":be,"../assets/cubeMap2/posz.png":Be,"../assets/particle.png":ze,"../assets/star3.png":Ee,"../assets/宇宙4K (5).hdr":Me})[`../assets/${u}`],import.meta.url).href}class Qe{constructor(e){this.parent=e.parent,this.target=e.target,this.callback=e.callback,this.devicePixelRatio=window.devicePixelRatio,this.width=this.target.offsetWidth*this.devicePixelRatio,this.height=this.target.offsetHeight*this.devicePixelRatio,this.clock=new _,this.renderer=new Q({powerPreference:"high-performance",antialias:!1,stencil:!1,depth:!1,preserveDrawingBuffer:!0}),this.renderer.setPixelRatio(window.devicePixelRatio),this.renderer.setSize(this.width,this.height),this.renderer.toneMapping=k,this.renderer.outputColorSpace=U,this.target.appendChild(this.renderer.domElement),this.scene=new j,this.blackColor=new r(0),this.scene.background=this.blackColor,this.camera=new G(45,this.width/this.height,.1,100),this.camera.position.set(15,0,15),this.control=new L(this.camera,this.renderer.domElement),this.control.enableDamping=!0,this.composerInit(),this.geometryInit(),this.paramsInit(),this.particleInit(),this.paneInit(),this.assetsInit()}composerInit(){this.composer1=new g(this.renderer),this.composer2=new g(this.renderer),this.renderPass=new K(this.scene,this.camera),this.unrealBloomPass=new V(new T(this.height,this.width),.26,.4,.2),this.bloomPass=new O(new v({uniforms:{tDiffuse:{value:null},uBloomTexture:{value:this.composer1.renderTarget2.texture},uStrength:{value:12}},vertexShader:Se,fragmentShader:De})),this.smaaPass=new N(this.width,this.height),this.outPass=new q,this.composer1.addPass(this.renderPass),this.composer1.addPass(this.unrealBloomPass),this.composer1.renderToScreen=!1,this.composer2.addPass(this.renderPass),this.composer2.addPass(this.bloomPass),this.composer2.addPass(this.smaaPass),this.composer2.addPass(this.outPass)}paramsInit(){this.autoDissolve=!1,this.dissolving=!0,this.geoIdx=0,this.geoLength=this.geometries.length,this.meshColor="#"+new r("#636363").getHexString(),this.edgeColor="#"+new r("#4d9bff").getHexString(),this.particleColor="#"+new r("#4d9bff").getHexString(),this.particleData={particleSpeedFactor:.02,velocityFactor:{x:2.5,y:2},waveAmplitude:0},this.uniforms={uColor:{value:new r(5086207)},uFreq:{value:.45},uAmp:{value:16},uProgress:{value:-7},uEdge:{value:.8}}}beforeCompile(){this.meshPhysicalMaterial.onBeforeCompile=e=>{e.uniforms={...e.uniforms,...this.uniforms},e.vertexShader=e.vertexShader.replace("#include <common>",`#include <common>
            ...
            `)}}geometryInit(){this.sphere=new H(4.5,140,140),this.teaPot=new Y(3,32),this.torus=new J(3,1.5,140,140),this.torusKnot=new Z(2.5,.8,140,140),this.geoNames=["TorusKnot","Tea Pot","Sphere","Torus"],this.geometries=[this.torusKnot,this.teaPot,this.sphere,this.torus],this.meshGeometry=this.geometries[0],this.meshPhysicalMaterial=new W({color:new r("#636363"),metalness:2,roughness:0,side:X}),this.beforeCompile(),this.mesh=new d(this.meshGeometry,this.meshPhysicalMaterial),this.scene.add(this.mesh)}particleInit(){this.particleMaterial=new v({transparent:!0,blending:$,uniforms:{uTexture:{value:this.particleTexture},uPixelDensity:{value:this.renderer.getPixelRatio()},uProgress:{value:this.uniforms.uProgress.value},uEdge:{value:.8},uAmp:{value:16},uFreq:{value:.25},uBaseSize:{value:80},uColor:{value:new r("#4d9bff")}},vertexShader:Fe,fragmentShader:Re}),this.particleMesh=new ee(this.meshGeometry,this.particleMaterial),this.scene.add(this.particleMesh),this.particleCount=this.meshGeometry.attributes.position.count,this.particleMaxOffsetArr=new Float32Array(this.particleCount),this.particleInitPosArr=new Float32Array(this.meshGeometry.getAttribute("position").array),this.particleCurrPosArr=new Float32Array(this.meshGeometry.getAttribute("position").array),this.particleVelocityArr=new Float32Array(this.particleCount*3),this.particleDistArr=new Float32Array(this.particleCount),this.particleRotationArr=new Float32Array(this.particleCount);for(let e=0;e<this.particleCount;e++){let t=e*3,i=e*3+1,s=e*3+2;this.particleMaxOffsetArr[e]=Math.random()*5.5+1.5,this.particleVelocityArr[t]=Math.random()*.5+.5,this.particleVelocityArr[i]=Math.random()*.5+.5,this.particleVelocityArr[s]=Math.random()*.1,this.particleDistArr[e]=.001,this.particleRotationArr[e]=Math.random()*Math.PI*2}this.setAttribute()}updateParticle(){for(let e=0;e<this.particleCount;e++){let t=e*3+0,i=e*3+1,s=e*3+2,n=this.particleVelocityArr[e*3+0],a=this.particleVelocityArr[e*3+1],o=this.particleVelocityArr[e*3+2];n*=this.particleData.velocityFactor.x,a*=this.particleData.velocityFactor.y;const h=this.particleCurrPosArr[e*3+0],c=this.particleCurrPosArr[e*3+1];let w=Math.sin(c*2)*(.8+this.particleData.waveAmplitude),I=Math.sin(h*2)*(.6+this.particleData.waveAmplitude),P=Math.sin(c*5)*(.2+this.particleData.waveAmplitude),b=Math.sin(h*1)*(.9+this.particleData.waveAmplitude),B=Math.sin(c*8)*(.8+this.particleData.waveAmplitude),z=Math.sin(h*5)*(.6+this.particleData.waveAmplitude),E=Math.sin(c*3)*(.8+this.particleData.waveAmplitude),M=Math.sin(h*7)*(.6+this.particleData.waveAmplitude),S=w+P+B+E,D=I+b+z+M;n+=S,a+=D,n*=Math.abs(this.particleData.particleSpeedFactor),a*=Math.abs(this.particleData.particleSpeedFactor),o*=Math.abs(this.particleData.particleSpeedFactor),this.particleCurrPosArr[t]+=n,this.particleCurrPosArr[i]+=a,this.particleCurrPosArr[s]+=o;const F=new x(this.particleInitPosArr[t],this.particleInitPosArr[i],this.particleInitPosArr[s]),R=new x(this.particleCurrPosArr[t],this.particleCurrPosArr[i],this.particleCurrPosArr[s]),m=F.distanceTo(R);this.particleDistArr[e]=m,this.particleRotationArr[e]+=.01,m>this.particleMaxOffsetArr[e]&&(this.particleCurrPosArr[t]=this.particleInitPosArr[t],this.particleCurrPosArr[i]=this.particleInitPosArr[i],this.particleCurrPosArr[s]=this.particleInitPosArr[s])}this.setAttribute()}setAttribute(){this.meshGeometry.setAttribute("aOffset",new l(this.particleMaxOffsetArr,1)),this.meshGeometry.setAttribute("aCurrentPos",new l(this.particleCurrPosArr,3)),this.meshGeometry.setAttribute("aVelocity",new l(this.particleVelocityArr,3)),this.meshGeometry.setAttribute("aDist",new l(this.particleDistArr,1)),this.meshGeometry.setAttribute("aAngle",new l(this.particleRotationArr,1))}floatMeshes(e){this.mesh.position.set(0,Math.sin(e*2)*.5,0),this.particleMesh.position.set(0,Math.sin(e*2)*.5,0)}async assetsInit(){this.manager=new te,this.cubeTexture=await new ie(this.manager).loadAsync(C("宇宙4K (5).hdr")),this.cubeTexture.mapping=se,this.scene.background=this.cubeTexture,this.scene.environment=this.cubeTexture,this.particleTexture=await new ne(this.manager).load(C("star3.png")),this.manager.onLoad=async()=>{console.info("%c资源加载完成!✅","color:#fff;background-color:red"),this.callback(),this.particleInit(),this.animation()}}resizeRenderer(){const e=this.target.offsetWidth*this.devicePixelRatio,t=this.target.offsetHeight*this.devicePixelRatio,i=this.width!==e||this.height!==t;return i&&(this.renderer.setSize(e,t),this.renderPass.setSize(e,t),this.outPass.setSize(e,t),this.unrealBloomPass.setSize(e,t),this.composer1.setSize(e,t),this.composer2.setSize(e,t),this.width=e,this.height=t),i}animateDissolve(){if(!this.autoDissolve)return;let e=this.uniforms.uProgress;this.dissolving?e.value+=.08:e.value-=.08,e.value>20&&this.dissolving&&(this.dissolving=!1,this.geoIdx++,this.handleChange(this.geometries[this.geoIdx%this.geoLength]),this.meshBlade.value=this.geometries[this.geoIdx%this.geoLength]),e.value<-20&&!this.dissolving&&(this.dissolving=!0),this.progressBinding.controller.value.setRawValue(e.value)}animation(){let e=this.clock.getElapsedTime();this.renderer.setAnimationLoop(()=>this.animation()),this.control.update(),this.updateParticle(),this.floatMeshes(e),this.animateDissolve(),this.resizeRenderer()&&(this.camera.aspect=this.width/this.height,this.camera.updateProjectionMatrix()),this.scene.background=this.blackColor,this.composer1.render(),this.scene.background=this.cubeTexture,this.composer2.render()}handleChange(e){this.scene.remove(this.mesh),this.scene.remove(this.particleMesh),this.meshGeometry=e,this.mesh=new d(this.meshGeometry,this.meshPhysicalMaterial),this.particleInit(),this.scene.add(this.mesh)}paneInit(){this.pane=new re({container:this.parent}),this.controller=this.pane.addFolder({title:"控制台",expanded:!0}),this.bloomFolder=this.controller.addFolder({title:"辉光",expanded:!0}),this.bloomFolder.addBinding(this.bloomPass.uniforms.uStrength,"value",{min:1,max:20,step:.01,label:"强度"}).on("change",e=>{this.bloomPass.uniforms.uStrength.value=e.value}),this.meshBlade=this.pane.addBlade({view:"list",label:"物体切换",options:[{text:"TorusKnot",value:this.torusKnot},{text:"Tea Pot",value:this.teaPot},{text:"Sphere",value:this.sphere},{text:"Torus",value:this.torus}],value:this.torusKnot}).on("change",e=>{this.handleChange(e.value)}),this.meshFolder=this.controller.addFolder({title:"物体",expanded:!0}),this.meshFolder.addBinding(this.mesh,"visible",{label:"显示物体"}).on("change",e=>{this.mesh.visible=e.value}),this.progressBinding=this.meshFolder.addBinding(this.uniforms.uProgress,"value",{min:-20,max:20,step:1e-4,label:"进度"}).on("change",e=>{this.uniforms.uProgress.value=e.value,this.particleMaterial.uniforms.uProgress.value=e.value}),this.meshFolder.addBinding(this,"autoDissolve",{label:"播放动画"}).on("change",e=>{this.autoDissolve=e.value}),this.meshFolder.addBinding(this.uniforms.uEdge,"value",{min:.1,max:8,step:.001,label:"边缘宽度"}).on("change",e=>{this.uniforms.uEdge.value=e.value}),this.meshFolder.addBinding(this.uniforms.uFreq,"value",{min:.001,max:2,step:.001,label:"频率"}).on("change",e=>{this.uniforms.uFreq.value=e.value}),this.meshFolder.addBinding(this.uniforms.uAmp,"value",{min:.1,max:50,step:.001,label:"振幅"}).on("change",e=>{this.uniforms.uAmp.value=e.value}),this.meshFolder.addBinding(this,"meshColor",{label:"物体颜色"}).on("change",e=>{this.meshPhysicalMaterial.color.set(e.value)}),this.meshFolder.addBinding(this,"edgeColor",{label:"边缘颜色"}).on("change",e=>{this.uniforms.uColor.value.set(e.value)}),this.particleFolder=this.controller.addFolder({title:"粒子",expanded:!0}),this.particleFolder.addBinding(this.particleMesh,"visible",{label:"显示粒子"}).on("change",e=>{this.particleMesh.visible=e.value}),this.particleFolder.addBinding(this.particleMaterial.uniforms.uBaseSize,"value",{min:.1,max:3e3,step:.01,label:"粒子大小"}).on("change",e=>{this.particleMaterial.uniforms.uBaseSize.value=e.value}),this.particleFolder.addBinding(this,"particleColor",{label:"粒子颜色"}).on("change",e=>{this.particleMaterial.uniforms.uColor.value.set(e.value)}),this.particleFolder.addBinding(this.particleData,"particleSpeedFactor",{min:.001,max:.1,step:.001,label:"粒子漂浮速度"}).on("change",e=>{this.particleData.particleSpeedFactor=e.value}),this.particleFolder.addBinding(this.particleData,"waveAmplitude",{min:0,max:5,step:.01,label:"粒子波幅"}).on("change",e=>{this.particleData.waveAmplitude=e.value}),this.particleFolder.addBinding(this.particleData,"velocityFactor",{expanded:!0,picker:"inline",label:"粒子方向"}).on("change",e=>{this.particleData.velocityFactor=e.value})}destroy(){this.scene.traverse(e=>{var t;e instanceof d&&((t=e.geometry)==null||t.dispose(),Object.values(e.material).forEach(i=>{i&&typeof i.dispose=="function"&&i.dispose()}))}),this.composer1.dispose(),this.composer2.dispose(),this.renderer.dispose()}}const ke={class:"dissipate"},Ue={class:"load"},je={__name:"index",setup(u){const e=f(!1);f(0);let t=null;const i=s=>{e.value=!0};return ae(()=>{t=new Qe({parent:document.querySelector(".dissipate"),target:document.querySelector(".canvas"),callback:i})}),oe(()=>{t.destroy(),t=null,console.info("%c消融特效-销毁😁","color:#fff;background-color:red")}),(s,n)=>(y(),A("div",ke,[p("div",{class:ue(["loading",{loadOk:e.value}])},[p("div",Ue,[(y(),A(le,null,he("LOADING...",(a,o)=>p("span",{key:o,style:ce("--i:"+o)},pe(a),5)),64))])],2),n[0]||(n[0]=p("div",{class:"canvas"},null,-1))]))}},Ve=de(je,[["__scopeId","data-v-f3f7da80"]]);export{Ve as default};
