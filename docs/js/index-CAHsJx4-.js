...
`;function S(o){const e={...o};return"defines"in o&&(e.defines={...o.defines}),"uniforms"in o&&(e.uniforms=te.clone(o.uniforms)),e}const st={defines:{X_IS_EVEN:1,Y_IS_EVEN:1},uniforms:{map:{value:null},originalMapSize:{value:new _},parentMapSize:{value:new _},parentLevel:{value:0}},vertexShader:`
		varying vec2 vUv;
		void main() {
	...
	`},Y=new r;class rt{constructor(e){e||(e=`
			...
			`);const t=S(st);t.fragmentShader=t.fragmentShader.replace(/<mipmap_logic>/g,e);const i=new Array(4);i[0]=new g(S(t)),i[0].defines.X_IS_EVEN=0,i[0].defines.Y_IS_EVEN=0,i[1]=new g(S(t)),i[1].defines.X_IS_EVEN=1,i[1].defines.Y_IS_EVEN=0,i[2]=new g(S(t)),i[2].defines.X_IS_EVEN=0,i[2].defines.Y_IS_EVEN=1,i[3]=new g(S(t)),i[3].defines.X_IS_EVEN=1,i[3].defines.Y_IS_EVEN=1;const n=new D;n.texture.minFilter=T,n.texture.magFilter=T,this._swapTarget=n,this._copyQuad=new Q(new g(ie)),this._mipQuad=new Q(null),this._mipMaterials=i}update(e,t,i,n=!1){e.isWebGLRenderTarget&&(e=e.texture);const l=i.autoClear,p=i.getClearAlpha(),m=i.getRenderTarget();i.getClearColor(Y);const c=this._copyQuad,a=this._mipQuad,s=this._swapTarget,M=this._mipMaterials;let A,u;n?(A=V.floorPowerOfTwo(e.image.width),u=V.floorPowerOfTwo(e.image.height)):(A=Math.floor(e.image.width),u=Math.floor(e.image.height));const P=Math.floor(A*1.5),y=Math.floor(u);t.setSize(P,y),s.texture.type!==t.texture.type?(s.dispose(),s.copy(t),s.texture.image={...s.texture.image}):s.setSize(P,y),i.autoClear=!1,i.setClearColor(0),i.setClearAlpha(),c.material.uniforms.tDiffuse.value=e,c.camera.setViewOffset(A,u,0,0,P,y),i.setRenderTarget(t),i.clear(),c.render(i),i.setRenderTarget(s),i.clear(),c.render(i);let I=A,v=u,R=0;for(;I>1&&v>1;){const K=(I%2===0?1:0)|(v%2===0?2:0),x=M[K];x.uniforms.map.value=s.texture,x.uniforms.parentLevel.value=R,x.uniforms.parentMapSize.value.set(I,v),x.uniforms.originalMapSize.value.set(A,u),a.material=x,I=Math.floor(I/2),v=Math.floor(v/2);const q=y-2*v;i.setRenderTarget(t),a.camera.setViewOffset(I,v,-A,-q,P,y),a.render(i),i.setRenderTarget(s),x.uniforms.map.value=t.texture,a.render(i),R++}return i.setRenderTarget(m),i.setClearAlpha(p),i.setClearColor(Y),i.autoClear=l,R+1}dispose(){this._swapTarget.dispose(),this._mipQuad.dispose(),this._copyQuad.dispose(),this._mipMaterials.forEach(e=>e.dispose())}}var lt=`uniform float iTime;
