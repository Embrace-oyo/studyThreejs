 ...
 * @author kongjianqiu
 * @description
 * @created 2024/4/12 09:31:28
 */
import {PackedMipMapGenerator} from '@/util/mipmap/PackedMipMapGenerator'
...
import dynamicEnvVertexShader from "../glsl/su7/DynamicEnv/vert.glsl";
import dynamicEnvFragmentShader from "../glsl/su7/DynamicEnv/frag.glsl";
import reflecFloorVertexShader from "../glsl/su7/ReflecFloor/vert.glsl";
import reflecFloorFragmentShader from "../glsl/su7/ReflecFloor/frag.glsl";
import speedUpVertexShader from "../glsl/su7/Speedup/vert.glsl";
import speedUpFragmentShader from "../glsl/su7/Speedup/frag.glsl";

export default class ModelClass {
    constructor(option) {
        if (!option.parent) {
            console.error('未接收到父容器DOM!')
            return
        }
        if (!option.canvas) {
            console.error('未接收到绘画容器DOM!')
            return
        }
        if (!option.modelUrl) {
            console.error('请传入模型链接!')
            return
        }
        this.option = {
            scene: {
        ...
        this.width = this.option.canvas.offsetWidth || window.innerWidth * window.devicePixelRatio
        this.height = this.option.canvas.offsetHeight || window.innerHeight * window.devicePixelRatio
    ...
    baseInit() {
        ...
        this.scene.background = new THREE.Color(this.option.scene.background);
        // this.scene.fog = new THREE.Fog(this.option.scene.fogBackground, this.option.scene.fogNear, this.option.scene.fogFar);
        // 创建相机
        this.camera = new THREE.PerspectiveCamera(this.option.camera.cameraFov, this.width / this.height, this.option.camera.cameraNear, this.option.camera.cameraFar);
        this.camera.position.set(this.option.camera.x, this.option.camera.y, this.option.camera.z);
        this.camera.lookAt(this.option.camera.lookAtX, this.option.camera.lookAtY, this.option.camera.lookAtZ)
    ...
    assetsManage() {
        ...
        // 环境贴图
        this.rgbeLoader1 = new RGBELoader(this.manager).setDataType(THREE.HalfFloatType).load('./texture/t_env_night.hdr')
        this.rgbeLoader2 = new RGBELoader(this.manager).setDataType(THREE.HalfFloatType).load('./texture/t_env_light.hdr')
        // 装饰贴图
        this.TextureLoader = new THREE.TextureLoader(this.manager)
        this.textureLoader1 = this.TextureLoader.load("./texture/t_startroom_ao.raw.jpg")
        this.textureLoader1.channel = 1;
        this.textureLoader1.flipY = false;
        this.textureLoader1.colorSpace = THREE.LinearSRGBColorSpace;
        this.textureLoader2 = this.TextureLoader.load("./texture/t_startroom_light.raw.jpg")
        this.textureLoader2.channel = 1;
        this.textureLoader2.flipY = false
        this.textureLoader2.colorSpace = THREE.SRGBColorSpace;
        this.textureLoader3 = this.TextureLoader.load("./texture/t_floor_normal.webp")
        ...
        this.textureLoader4 = this.TextureLoader.load("./texture/t_floor_roughness.webp")
        ...
        this.textureLoader5 = this.TextureLoader.load("./texture/t_car_body_AO.raw.jpg")
        ...
        this.textureLoader6 = new THREE.TextureLoader(this.manager).load('https://pmndrs.github.io/postprocessing/public/demo/textures/perturb.jpg')
        // 装饰模型
        const startLoader = new GLTFLoader(this.manager).setDRACOLoader(this.dracoLoader)
        this.startRoomLoader = startLoader.loadAsync('./model/sm_startroom.raw.gltf')
        // 汽车模型
        ...
        this.carLoader = carLoader.loadAsync('./model/sm_car.gltf')
        // speedup模型
        const speedupLoader = new GLTFLoader(this.manager).setDRACOLoader(this.dracoLoader).setMeshoptDecoder(meshoptDecoder);
        this.speedupLoader = speedupLoader.loadAsync('./model/sm_speedup.gltf')
        // 其他
        this.manager.onLoad = async () => {
            console.log('资源加载完成!');
            this.option.progressCallback()
            this.FBOInit()
            this.QUADInit()
    ...
    postEA() {
        ...
        const gui = new GUI({container: this.parent})
        gui.domElement.style.position = 'absolute'
        gui.domElement.style.right = '0'
        gui.domElement.style.top = '0'
        gui.domElement.style.zIndex = '999'
        const uniforms = glitchEffect.uniforms
        const params = {
            "custom pattern": true,
            columns: uniforms.get("columns").value
        }
        gui.add(glitchEffect, "mode", POSTPROCESSING.GlitchMode).onChange((value) => {
            glitchEffect.mode = Number(value);
            console.log(glitchEffect)
        });
        gui.add(params, "custom pattern").onChange((value) => {

            if(value) {

                glitchEffect.setPerturbationMap(glitchEffect.getPerturbationMap());

            } else {

                glitchEffect.setPerturbationMap(glitchEffect.generatePerturbationMap(64));

            }

        });
        gui.add(glitchEffect.delay, "x", 0.0, 2.0, 0.001).name('min delay').onChange((value) => {
            glitchEffect.delay.x = value;
        });
        gui.add(glitchEffect.delay, "y", 2.0, 4.0, 0.001).name('max delay').onChange((value) => {
            glitchEffect.delay.y = value;
        });
        gui.add(glitchEffect.duration, "x", 0.0, 0.6, 0.001).name('min duration').onChange((value) => {
            glitchEffect.duration.x = value;
        });
        gui.add(glitchEffect.duration, "y",  0.6, 1.8, 0.001).name('max duration').onChange((value) => {
            glitchEffect.duration.x = value;
        });

        const folder = gui.addFolder("Strength");
        folder.add(glitchEffect.strength, "x", 0.0, 1.0, 0.001).name('weak glitches').onChange((value) => {

            glitchEffect.strength.x = value;

        });

        folder.add(glitchEffect.strength, "y", 0.0,1.0, 0.001).name('strong glitches').onChange((value) => {

            glitchEffect.strength.y = value;

        });
        folder.open();

        gui.add(glitchEffect, "ratio", 0.0, 1.0, 0.001).name('glitch ratio').onChange((value) => {

            glitchEffect.ratio = Number.parseFloat(value);

        });
        gui.add(params, "columns", 0.0, 0.5, 0.001).name('columns').onChange((value) => {

            glitchEffect.uniforms.get("columns").value = value;
    ...
    statsInit() {
        ...
        this.option.parent.appendChild(this.stats.dom);
    }
