...
void main(){
    ...
    // 波浪高度
    float elevation = sin(modelPosition.x * u_bigWavesFrequency.x + u_time * u_bigWavesSpeed.x) * sin(modelPosition.z * u_bigWavesFrequency.y + u_time * u_bigWavesSpeed.y) * u_bigWavesElevation;
    for (float i = 1.0; i <= u_smallIterations; i++){
