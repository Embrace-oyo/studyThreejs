...
void main() {
    ...
    float noise = cnoise(vec3(p * 10.0, 0.0));
    float pr = progress + noise * 0.1;
    float d = sdCircle(p, pr * sqrt(2.4));
    float c = smoothstep(-0.1, -0.05, d);
    vec3 t1 = texture2D( tDiffuse2, vUv ).xyz;
    vec3 t2 = texture2D( tDiffuse1, vUv ).xyz;
    vec3 col = mix(t1, t2, c);
