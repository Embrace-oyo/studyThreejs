...
uniform float iTime;
uniform float audioData[256];  // 音频频率数据
//
...
void main() {
    v_uv = uv;
    vec3 p = position;
    p+= audioData[int(position)];
    float noise = cnoise(p + iTime);
//    float noise = cnoise(p + audioData[int(position.x)]);
    p += noise * normal * 0.5;
    float height = sin(position.x * 0.1) * cos(position.z * 0.1);
    height += noise * 5.0;
    v_height = height;
