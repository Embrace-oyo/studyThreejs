...
//glsl
import baseVertex from '@/views/pages/fluid/glsl/baseVertex.glsl'
import baseFragment from "@/views/pages/fluid/glsl/baseFragment.glsl";
import mixFragment from '@/views/pages/fluid/glsl/mixFragment.glsl'
import blurFragment from '@/views/pages/fluid/glsl/blurFragment.glsl'
import outFragment from "@/views/pages/fluid/glsl/outFragment.glsl";
...
export default class FluidPass {
    ...
    constructor(config) {
        ...
        this.targetCamera.position.copy(this.base.camera.position);
        this.targetMesh = new THREE.Mesh(this.targetGeometry);
        this.targetMesh.frustumCulled = !1;
        this.dateTime = performance.now();
        this.randomMouseXY = new THREE.Vector2();
        this.meshInit();
        this.guiInit();
        this.t1 = new gsap.timeline({});
        this.randomMouse();
        this.timer = setInterval(() => this.randomMouse(), 200, false);
    ...
    randomMouse() {
        this.t1.to(this.randomMouseXY, {
            x: Math.random() * this.target.offsetWidth,
            y: Math.random() * this.target.offsetHeight,
            duration: 0.5,
            ease: "power2.inOut",
            onUpdate: () => {
                this.mouseXY.set(this.randomMouseXY.x / this.target.offsetWidth * 2 - 1, 1 - this.randomMouseXY.y / this.target.offsetHeight * 2);
                this.mousePixelXY.set(this.randomMouseXY.x, this.randomMouseXY.y);
                this.deltaXY.copy(this.mouseXY).sub(this._prevMouseXY)
                this._prevMouseXY.copy(this.mouseXY)
                this.hasMoved = this.deltaXY.length() > 0
            },
    ...
    frameOne(renderer, writeBuffer, readBuffer) {
    ...
    render(renderer, writeBuffer, readBuffer) {
        ...
        // 流体绘制
        this.frameOne(renderer, writeBuffer, readBuffer);
        // 模糊效果绘制
